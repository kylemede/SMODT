...
{
	generalTools GT;
	int numModDatasets = 10000;
	string modDatasetFilename = "mod10000Dataset.dat";
...
//    	cout<<"Warning: (acceptedCounter) "<<acceptedCounter<< " != "<<totalAccepted<<" (totalAccepted)"<<endl;
	...
	{
		//cout<<"starting to load up RV data obj"<<endl; //$$$$$$$$$$$$$$
		string RV100modDataFilename = SSO.settings_and_InputDataDir + modDatasetFilename;
		cout<<"Using RV1000modDataFilename:"<<RV100modDataFilename<<endl;
	...
	for (int modDatasetNum = 0;modDatasetNum<numModDatasets;++modDatasetNum)
	{
...
//		cout<<"\n\n#### Size of MCMCOFO.RVdo.epochs_RV[0] = "<<MCMCOFO.RVdo.epochs_RV[0].size()<<endl;
//		cout<<"RVdoMOD100.epochs_RV[0]  = "<<RVdoMOD100.epochs_RV[0].size()<<endl;
//		cout<<"MCMCOFO.RVdo.epochs_RV  = "<<MCMCOFO.RVdo.epochs_RV.size()<<endl;

		MCMCOFO.paramChangeStepSizePercent = 0.1;
		MCMCOFO.sigmaPercent = 2.45;

		if (SAOFO.chiSquareMin>SSO.chiSquaredMax)
		...
		else
		{
			...
			timeStr = GT.timeStr(timeElapsed);
			ss<<"\nSimulator took "<<timeStr<<" to complete"<<endl;
//			if ( timeElapsed < 60.0 )
...
//			}
	...
	ss<< "\n\n"<<endl;
	ss<<"**************************************************************************"<<endl;
	ss<<"****************** TOTAL MOD DATASET SIMULATION COMPLETE *************"<<endl;
	ss<<"**************************************************************************"<<endl;
	timeStr = GT.timeStr(timeElapsed);
	ss<<"\nSimulator took "<<timeStr<<" to complete"<<endl;
//	if ( timeElapsed < 60.0 )
...
//	}
	...
	GT.fileWriter(ODTall);

	GT.logFileWriter(ODTall_data_filename, LOGlinesALL);
