    ...
    for ( int sample=1; sample<(numSamples+1); sample++)
    {
        ...
        if ( printCount==printTime )
        {
            ...
            if ( silent==false )
            {
                ...
                cout << "latest reduced chi-square = " << reduced_chiSquared_total_cur <<endl;
        ...
        MEOCIT.inclination_deg = uniformRandNumber(inclination_degMIN, inclination_degMAX);
        MEOCIT.longAN_deg = uniformRandNumber(longAN_degMIN, longAN_degMAX);
        MEOCIT.argPeri_deg = uniformRandNumber(argPeri_degMIN, argPeri_degMAX);
        MEOCIT.e = uniformRandNumber(eMIN, eMAX);
        MEOCIT.period = uniformRandNumber(periodMIN, periodMAX); //  [yrs]
        double Tmin = epochs[0]-MEOCIT.period*365.0;
        double Tmax = epochs[0];
        MEOCIT.T = uniformRandNumber(Tmin, Tmax); // thus between a full period before first observation and the time of first observation
        MEOCIT.a_total = uniformRandNumber(a_totalMIN, a_totalMAX);
		...
		if ( reduced_chiSquared_total_cur<=chiSquaredMax )
		{
			acceptedCounter +=1;
