...
int main()
{
    ...
    //********************************************************************
    double chiSquaredMax = 50.0 ;
    double numSamplePrints = 100.0   ;
    double targetVariance = 75.0;
    double totalTunings;
    if ( numSamples>1e6)
    	totalTunings = 10000;
    else if ((numSamples<=1e6)&&(numSamples>100))
    	totalTunings = 100;
    else
    	totalTunings = 1;
    int tuningSamples = 500;//(int)(numSamples/totalTunings);
    cout<< "tuningSamples = "<<tuningSamples<<endl;//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    double sigmaMAXpercent = 0.5;//ie. the whole range

    // Ranges for acceptable random number inputs ######
    double longAN_degMIN = 133.25; // [deg]
    double longAN_degMAX = 133.45 ;// [deg]
    double eMIN = 0.14 ;
    double eMAX = 0.17 ;
    double periodMIN = 9.55; // [yrs]
    double periodMAX = 10.0; // [yrs]
    double inclination_degMIN = 95.70; // [deg]
    double inclination_degMAX = 96.0; // [deg]
    double argPeri_degMIN = 67.0; // [deg]
    double argPeri_degMAX = 80.0; // [deg]
    double a_totalMIN = 2.17;
    double a_totalMAX = 2.22 ;
    // MEASURED VALUES ########################
	...
	SDT.longAN_degs.push_back(133.37);//(longAN_degMAX-longAN_degMIN)/2.0);
	SDT.es.push_back(0.145);//(eMAX-eMIN)/2.0);
	double TMAX = epochs[0]-periodMAX*365.0;
	double TMIN = epochs[0];
	SDT.Ts.push_back(2451124.1);//(TMAX-TMIN)/2.0);
	SDT.periods.push_back(9.74);//(periodMAX-periodMIN)/2.0);
	SDT.inclination_degs.push_back(95.9);//(inclination_degMAX-inclination_degMIN)/2.0);
	SDT.argPeri_degs.push_back(73.85);//(argPeri_degMAX-argPeri_degMIN)/2.0);
	SDT.a_totals.push_back(2.18);//(a_totalMAX-a_totalMIN)/2.0);
    ...
    int hundredthCounter = 0;
    double acceptRateLatest;
    double reduced_chiSquared_total_cur;
    double original_chiSquared_total_cur;
    double latest_accepted_chiSquared = 0;
    double one_over_nu;
    double chiSquareMin = chiSquaredMax;
    int bestOrbit = 0;
    int totalCounter = 0;
    double probLast = gaussianDist(chiSquaredMax, 1.0, targetVariance);
    double sigma, max, min;
    // set up initial sigma for tunning
    double sigma_latest = 0.2;
    ...
    for ( int sample=1; sample<(numSamples+1); sample++)
    {
        ...
        if ( printCount==printTime )
        {
            ...
            if ( silent==false )
            {
				...
				cout << "latest acceptance rate = " <<acceptRateLatest<<endl;
        ...
        if ( (SDT.inclination_degs.back()+sigma)>=inclination_degMAX )
        {
        	max = inclination_degMAX;
        	min = inclination_degMAX-(sigma*2.0);
        }
        else if ( (SDT.inclination_degs.back()-sigma)<=inclination_degMIN )
        {
        ...
        {
        	max = SDT.inclination_degs.back() + sigma;
			min = SDT.inclination_degs.back() - sigma;
        }
		...
		if ( (SDT.longAN_degs.back()+sigma)>=longAN_degMAX )
		{
			max = longAN_degMAX;
			min = longAN_degMAX-(sigma*2.0);
		}
		else if ( (SDT.longAN_degs.back()-sigma)<=longAN_degMIN )
		{
		...
		{
			max = SDT.longAN_degs.back() + sigma;
			min = SDT.longAN_degs.back() - sigma;
		}
		...
		if ( (SDT.argPeri_degs.back()+sigma)>=argPeri_degMAX )
		{
			max = argPeri_degMAX;
			min = argPeri_degMAX-(sigma*2.0);
		}
		else if ( (SDT.argPeri_degs.back()-sigma)<=argPeri_degMIN )
		{
		...
		{
			max = SDT.argPeri_degs.back() + sigma;
			min = SDT.argPeri_degs.back() - sigma;
		}
		...
		if ( (SDT.es.back()+sigma)>=eMAX )
		{
			max = eMAX;
			min = eMAX-(sigma*2.0);
		}
		else if ( (SDT.es.back()-sigma)<=eMIN )
		{
			max = eMIN+(sigma*2.0);
			min = eMIN;
		}
		...
		{
			max = SDT.es.back() + sigma;
			min = SDT.es.back() - sigma;
		}
		...
		if ( (SDT.periods.back()+sigma)>=periodMAX )
		{
			max = periodMAX;
			min = periodMAX-(sigma*2.0);
		}
		else if ( (SDT.periods.back()-sigma)<=periodMIN )
		{
		...
		{
			max = SDT.periods.back() + sigma;
			min = SDT.periods.back() - sigma;
		}
		...
		if ( (SDT.Ts.back()+sigma)>=Tmax )
		{
			max = Tmax;
			min = Tmax-(sigma*2.0);
		}
		else if ( (SDT.Ts.back()-sigma)<=Tmin )
		{
			max = Tmin+(sigma*2.0);
			min = Tmin;
		}
		...
		{
			max = SDT.Ts.back() + sigma;
			min = SDT.Ts.back() - sigma;
		}
		...
		if ( (SDT.a_totals.back()+sigma)>=a_totalMAX )
		{
			max = a_totalMAX;
			min = a_totalMAX-sigma;
		}
		else if ( (SDT.a_totals.back()-sigma)<=a_totalMIN )
		{
		...
		{
			max = SDT.a_totals.back() + sigma;
			min = SDT.a_totals.back() - sigma;
		}
		...
		double probCur = gaussianDist(reduced_chiSquared_total_cur, 1.0, targetVariance);
		double probRatio = probCur/probLast;

		// Apply Metropolis-Hastings acceptance test
		double rightSide;
		if ( probRatio<1.0 )
			rightSide = 1.0;
		else
		...
		double alpha = uniformRandNumber(0.0, 1.0);
		...
		if ( alpha <= rightSide )
		{
			...
			if ( silent2==false )
			{
				cout<< "probCur = "<<probCur<<endl;
				cout<<"probLast = "<<probLast<<endl;
			}
			probLast = probCur;
			acceptedCounter +=1;
			...
			// increment the hundredth counter
			hundredthCounter += 1;

			if ( hundredthCounter == 100)
			{
				// reset the hundredthCounter
				hundredthCounter = 0;
		...
		if ( totalCounter==tuningSamples)
		{
			...
			// time to recalculate the acceptance rate and adjust the sigma value accordingly
			acceptRateLatest = (double)acceptedCounter/(double)tuningSamples;
			acceptedCounter = 0;
			totalCounter = 0;
			if ( sigma_latest< (sigmaMAXpercent-0.000001) )
			{
				if ( acceptRateLatest>=0.35 )
				{
					// ie. acceptance rate is too high, so raise sigma by 5%
					sigma_latest = sigma_latest+0.05;
					//cout<< "acceptance rate too low, so raising it by 0.05"<<endl;//$$$$$$$$$$$$$$$
				}
				else if ( acceptRateLatest<0.20)
				{
					// ie. acceptance rate is too low, so lower sigma by 5%
					if ( sigma_latest<=0.0500001)
					{
						// All ready below or at min, so leave it
						//cout<< "sigma all ready at min, so leaving it"<<endl;//$$$$$$$$$$$$$$$
					}
					else
						sigma_latest = sigma_latest-0.05;
					//cout<< "acceptance rate too high, so lowering it by 0.05"<<endl;//$$$$$$$$$$$$$$$
				}
			}
			else
			{
				// sigma value is all ready at max, so leave it.
				//cout<< "sigma all ready at max, so leaving it"<<endl;//$$$$$$$$$$$$$$$
			}

			if ( silent==false )
			{
				//cout<< "Finished a round of sigma tuning, New sigma = "<< sigma_latest<<endl;
			}
		}
