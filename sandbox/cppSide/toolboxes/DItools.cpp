...
//		SMT_in.Mass1 = Mass1;
//		SMT_in.Mass2 = Mass2;
//		SMT_out = semiMajorConverter(SMT_in);
...
//		cout<<"ERROR! no values for the two object masses were provided!!!"<<endl;
	...
	for ( int i=0; i<((int) epochs_DI.size()); i++ )
	{
		...
		double x_chi_squared = chiSquaredCalc(x_data, x_data_inv_var, OCRT.x_model);
        double y_chi_squared = chiSquaredCalc(y_data, y_data_inv_var, OCRT.y_model);
