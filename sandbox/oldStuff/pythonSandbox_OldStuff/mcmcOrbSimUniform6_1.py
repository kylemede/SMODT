...
with the changes understood after meeting with Tim Brant in January 2012.
    ...
    ACCEPTANCE RATE.
    THIS VERSION OF THE '6' VERSION HAS ONLY A SINGLE PARAMETER BEING VARIED AT A TIME
    AS THE ORIGINAL VERSION WHERE ALL 7 WERE CHANGING TOGETHER HAD POOR PERFORMANCE.
"""
...
sigmaPercent_MIN = 1.0
sigmaTuningIncrement = 0.1
tuningSamples = 5000
changeParamSamples = 100
...
argPeri_degMAX = 265.0#89.999 # [deg]          
a_totalMIN = 126.0
a_totalMAX = 144.0                                                                 
#### MEASURED VALUES ########################                                        
...
for curSample in range(1,numSamples+1):
    ...
    if printCount==printTime:
        ...
        if not silent:
            ...
            print "latest paramBeingVaried = ",paramBeingVaried
...
if not silent:
    ...
    print "overall acceptance rate = "+str(float(totalAcceptedCounter)/float(numSamples))
