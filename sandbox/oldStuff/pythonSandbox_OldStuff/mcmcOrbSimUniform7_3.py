...
import shutil
import time
from orbitToolbox import multiEpochOrbCalc, dictToFile
...
#    Sys_Dist_PC = 15.62
    ...
    epochs = paramSettingsDict['Data']['DI_epochs']#[2403065.5, 2405359.5, 2407064.5, 2408219.5, 24708927.5, 2411846.5, 2413509.5, 2414351.5, 2414465.5, 2415404.5, 2419263.5, 2423443.5, 2424260.5, 2425728.5, 2429753.5, 2431535.5, 2431959.5, 2434136.5, 2436256.5, 2437227.5, 2438866.5, 2440132.5, 2440707.5,      2448337.5, 2451143.5, 2451711.5, 2451945.5, 2454579.5, 2455589.5, 2455704.5]
    ...
    if startRandom:
        ...
        inclination_deg_latest = startParams[0]
        e_latest = startParams[1]
        longAN_deg_latest = startParams[2]
        period_latest = startParams[3]
        argPeri_deg_latest = startParams[4]
        a_total_latest = startParams[5]
        T_latest = startParams[6]
    ...
    for curSample in range(1,numSamples+1):
        ...
        Sys_Dist_proposed = rand.gauss(Sys_Dist_PC, Sys_Dist_Error)
        ...
        Mass1_proposed = rand.gauss(Mass1, Mass1_Error)
        
        # Draw a value for the companion star's (or planet) mass from a Gaussian 
        # centered on the previously found value and sigma=error in that value
        Mass2_proposed = rand.gauss(Mass2, Mass2_Error)
        ...
        #NOTE: I am ignoring the returned a1 and a2 for now and only saving the input a_total
        ...
        multiEpochOrbCalc(SA_arcsec_measured_REALs, SA_mean_errors, PA_deg_measured_REALs, PA_mean_errors,\
                       epochs, Sys_Dist_proposed, inclination_deg_proposed, longAN_deg_proposed, e_proposed, T_proposed, \
                       period_proposed, argPeri_deg_proposed, a_total_proposed, Mass1=Mass1_proposed, Mass2=Mass2_proposed, verbose=False)
    ...
    # write final string to file
    f.write(line)
    # Finished writting all samples, so close the file data is being written to
    f.close()
    ...
    if not silent:
        ...
        print "mean acceptance rate = ",resultsDict['meanAcceptanceRate']
    
    # record the time the chain finished and print
    endTime = time.clock()
    totalTime = (endTime-startTime) # in seconds
    if totalTime<60:
        totalTimeString = str(int(totalTime))+' seconds'
        print 'MCMC: The took '+totalTimeString+'to complete.\n'  ##### only print in 'silent' mode to track time
        resultsDict['totalTime'] = totalTimeString
    elif totalTime>3600:
        totalTime = totalTime/3600.0
        totalTimeString = str(int(totalTime))+' hours and '+str(int(60*((totalTime)-int(totalTime))))+' minutes'
        print 'MCMC: The took '+totalTimeString+' to complete.\n'  ##### only print in 'silent' mode to track time
        resultsDict['totalTime'] = totalTimeString
    else:
        totalTime = totalTime/60.0
        totalTimeString = str(int(totalTime))+' minutes and '+str(int(60*((totalTime)-int(totalTime))))+' seconds'
        print 'MCMC: The took '+totalTimeString+' to complete.\n'  ##### only print in 'silent' mode to track time
        resultsDict['totalTime'] = totalTimeString
