...
THIS VERSION HAS THE FIXED SIGMA VALUE AND VARIES A SINGLE PARAMETER AT A TIME, INSTEAD
 OF VARYING ALL THE PARAMETERS AT TIME WHICH HAD POOR PERFORMANCE.
"""
...
tuningSamples = 1000 # used to calculate acceptance rate only
changeParamSamples = 100 #number of times to perturb a single param before changing to a different one
#### Ranges for acceptable random number inputs ######                                        
longAN_degMIN = 220.0 # [deg]                                                        
longAN_degMAX = 236.0 # [deg]                                                        
eMIN = 0.82                                                                       
eMAX = 0.865                                                                      
periodMIN = 970.0 # [yrs]                                                                
periodMAX = 1115.0 # [yrs]                                                            
inclination_degMIN = 30.0 # [deg]                                                    
inclination_degMAX = 43.0 # [deg]                                                
argPeri_degMIN = 255.0 # [deg]                                                        
argPeri_degMAX = 265.0 # [deg]          
a_totalMIN = 126.0
a_totalMAX = 144.0                                        
#### MEASURED VALUES ########################                                        
...
if startRandom:
    ...
    T_latest = rand.uniform(epochs[-1]-period_latest*365.0, epochs[-1])
...
reduced_chiSquareMin = 10000.0
bestOrbit = 0
acceptRates = []
acceptRates.append(0)
tuningTotalCounter = 0
changeParamCounter = 0
...
for curSample in range(1,numSamples+1):
    ...
    if paramBeingVaried==0:
        ...
        sigmaRange = (sigmaPercent/100.0)*(sine_inc_MAX-sine_inc_MIN)
        ...
        # sigma is a percentage of total range, so convert its current percentage to a 
        # range value
        sigmaRange = (sigmaPercent/100.0)*(longAN_degMAX-longAN_degMIN)
        ...
        sigmaRange = (sigmaPercent/100.0)*(eMAX-eMIN)
        ...
        # sigma is a percentage of total range, so convert its current percentage to a 
        # range value
        sigmaRange = (sigmaPercent/100.0)*(periodMAX-periodMIN)
        ...
        TMIN = epochs[-1]
        sigmaRange = (sigmaPercent/100.0)*(TMAX-TMIN)
        ...
        # sigma is a percentage of total range, so convert its current percentage to a 
        # range value
        sigmaRange = (sigmaPercent/100.0)*(argPeri_degMAX-argPeri_degMIN)
        ...
        # sigma is a percentage of total range, so convert its current percentage to a 
        # range value
        sigmaRange = (sigmaPercent/100.0)*(a_totalMAX-a_totalMIN)
    ...
    changeParamCounter +=1
    # check if it is time to change the parameter being perturbed
    if changeParamCounter==changeParamSamples:
        changeParamCounter = 0
    ...
    if tuningTotalCounter==tuningSamples:
        ...
        acceptRates.append(float(tuningAcceptedCounter)/float(tuningSamples))
        tuningAcceptedCounter = 0
        tuningTotalCounter = 0
