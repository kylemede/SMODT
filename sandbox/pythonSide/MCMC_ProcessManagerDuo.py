...
class mcmcProcessManager(Process):
    ...
    '''
    def __init__(self, simSettingsFilename, filename, cplusplusCodeDir, simAnneal, loopedMCMC):
    ...
    def process(self, simSettingsFilename, filename, cplusplusCodeDir, simAnneal, loopedMCMC):
    
        if False:
            print 'Starting to run process for file title: '+filename
        
        if simAnneal:
            CplusplusCodeCALL = os.path.join(cplusplusCodeDir,'simAnnealOrbSimulator')
        elif loopedMCMC:
            CplusplusCodeCALL = os.path.join(cplusplusCodeDir,'looped_MCMCorbSimulator')
        else:
            CplusplusCodeCALL = os.path.join(cplusplusCodeDir,'MCMCorbSimulator')
...
def mcmcSimStarter(paramSettingsDict):
    """
    ...
    for processNumber in range(numProcesses):
        ...
        # start process
        master.append(mcmcProcessManager(simSettingsFilename=simSettingsFilename, 
    ...
    if (paramSettingsDict['simAnneal']==False)and(paramSettingsDict['makePosteriorsPlot']):
        cleanDataFilename=''
        if True:
            tools.plot.summaryPlotter2(dataFinalFilename, summaryPlotFile, weight=False, confLevels=True, nu=nu, plot4x1=plot4x1, TcStepping=paramSettingsDict['TcStepping'] )         
        if False:
    ...
    if os.path.exists(DIdatafilename)and ((paramSettingsDict['RVonly']==False)and(paramSettingsDict['makeOrbitPlots'])):
        ...
        tools.plot.orbitEllipsePlotterDuo(bestOrbit[0],bestOrbit[1],bestOrbit[4],bestOrbit[5],argPeriUse,bestOrbit[7],\
    ...
    if os.path.exists(RVdatafilename) and (paramSettingsDict['DIonly']==False)and(paramSettingsDict['makeOrbitPlots']):
        ...
        if paramSettingsDict['loopedMCMC']==False:
            # full orbit
            tools.plot.rvPlotterDuo(bestOrbit[1],bestOrbit[2],bestOrbit[3],bestOrbit[4],bestOrbit[5],argPeriUse,bestOrbit[7], \
