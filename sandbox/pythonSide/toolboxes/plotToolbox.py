...
def orbElementPlotter(plotFileTitle, longAN_degs, es, Ts, a1s, a2s, periods, inclination_degs, argPeri_degs,\
    ...
    """
    The old function to plot the orbital element posterior distributions.  Replaced by SummaryPlotter.
    """
    ########################## INS #####################
    # plot in's on first figure
    plt.figure(1,figsize=(20,7) ,dpi=300)
    plt.suptitle(plotFileTitle+' *Inputs*')
    plt.subplot(241)
    ...
    for epoch in range(0,numEpochs):
        ...
        plt.suptitle(plotFileTitle+' *Outputs of simulator for epoch '+str(epoch+1)+'*')
        plt.subplot(231)
...
def makeCleanSummaryPlot(outputDataFilename=''):
    ...
    This trimmed data file will then be plotted by summaryPlotter2.
    """
    verbose = True
    ...
    if os.path.exists(outputDataFilename):  
        ...
        summaryPlotter2(newDataFilename, newPlotFilename, weight=False, confLevels=True, normalize=True, nu=1, plot4x1=False, TcStepping=False)
        if verbose:
...
def summaryPlotter2(outputDataFilename, plotFilename, weight=False, confLevels=True, normalize=True, nu=1, plot4x1=False, TcStepping=False):
    """
    ...
    if MCMC==True, then a matching summary plot will also be made for the parameter values vs step, 
    ...
    if os.path.exists(outputDataFilename):  
        ...
        (CLevels,data,chiSquareds,bestDataVal) = confLevelFinderNEWdataVersion(outputDataFilename,paramColNum, returnData=True, returnChiSquareds=True, returnBestDataVal=True,fast=False)
        ...
        if not plot4x1:
            ...
            startTime1 = timeit.default_timer()
            (CLevels,data,bestDataVal) = confLevelFinderNEWdataVersion(outputDataFilename,paramColNum, returnData=True, returnChiSquareds=False, returnBestDataVal=True)
            endTime1 = timeit.default_timer()
            totalTime = (endTime1-startTime1) # in seconds
            totalTimeString = timeString(totalTime)
            s='Getting data took '+totalTimeString+' to complete.\n'  ##### only print in 'silent' mode to track time
...
def summaryPlotter2MCMC(outputDataFilename, plotFilename, nu=1, plot4x1=False, logFilename='',TcStepping=False):
    """
    This will plot a progress plot for each varying parameter in the file.
    """
    verbose = False
    ...
    if os.path.exists(outputDataFilename):   
        ...
        paramColNum = 8
        xlabel = 'ChiSquareds'
        (CLevels,data) = confLevelFinderNEWdataVersion(outputDataFilename,paramColNum, returnData=True, returnChiSquareds=False)
        log.write('\nCLevels for '+xlabel+':\n'+repr(CLevels)+'\n')
        yChunks = data[:chunkSize*numChunks].reshape((-1,chunkSize))
        yCenters = yChunks.mean(axis=1)
        subPlot2.fill_between(xCenters, min_env, max_env, color='gray',edgecolor='none', alpha=0.5)
...
def summaryPlotter2MCMCfunc(log,subPlot2,outputDataFilename,xlabel,paramColNum,xCenters,numChunks,chunkSize):
    ...
    """
    (CLevels,data,bestDataVal) = confLevelFinderNEWdataVersion(outputDataFilename,paramColNum, returnData=True, returnChiSquareds=False, returnBestDataVal=True,fast=True)
    log.write('\nCLevels for '+xlabel+':\n'+repr(CLevels)+'\n')
    if type(data)!=float:
        yChunks = data[:chunkSize*numChunks].reshape((-1,chunkSize))
        max_env = yChunks.max(axis=1)
...
def orbitEllipsePlotterDuo(longAN_deg, e, period, inc, argPeri_deg, a, sysDataDict, DIdataDict,\
                            xLabel='E ["]', yLabel='N ["]', \
...
#    print 'xStar = '+str(xStar) 
    ...
    for orb in range(0,len(longAN_deg)):
        ...
        for step in range(0,int(numSteps)):
            ...
            (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SA, PA, a1, a2) =\
                diTools.orbitCalculator2(t, sys_dist, inc[orb], longAN_deg[orb], e[orb], T, period[orb], argPeri_deg[orb], a[orb],\
    ...
    for orb in range(0,len(longAN_deg)):
        ...
        for t in ts:
            (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SA, PA, a1, a2) =\
                diTools.orbitCalculator2(t, sys_dist, inc[orb], longAN_deg[orb], e[orb], To[orb], period[orb], argPeri_deg[orb], a[orb],\
                                                            Mass1=1, Mass2=1, verbose=False)
    ...
    (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SAstart, PAstart, a1, a2) =\
            diTools.orbitCalculator2(0.0, sys_dist, inc[0], longAN_deg[0], e[0], 0.0, period[0], argPeri_deg[0], a[0],\
                                                        Mass1=1, Mass2=1, verbose=False)
    ...
    (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SAstart, PAstart, a1, a2) =\
            diTools.orbitCalculator2(((period[0]/4.0)*365.25), sys_dist, inc[0], longAN_deg[0], e[0], 0.0, period[0], argPeri_deg[0], a[0],\
                                                        Mass1=1, Mass2=1, verbose=False)
    ...
    (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SAhalf, PAhalf, a1, a2) =\
            diTools.orbitCalculator2(((period[0]/2.0)*365.25), sys_dist, inc[0], longAN_deg[0], e[0], 0.0, period[0], argPeri_deg[0], a[0],\
                                                        Mass1=1, Mass2=1, verbose=False)
    ...
    (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SAstart, PAstart, a1, a2) =\
            diTools.orbitCalculator2((3.0*(period[0]/4.0)*365.25), sys_dist, inc[0], longAN_deg[0], e[0], 0.0, period[0], argPeri_deg[0], a[0],\
                                                        Mass1=1, Mass2=1, verbose=False)
    ...
    (n, M_deg, E_latest_deg, TA_deg, Sep_Dist_AU_OP, SAstart, PAstart, a1, a2) =\
            diTools.orbitCalculator2(((period[0])*365.25), sys_dist, inc[0], longAN_deg[0], e[0], 0.0, period[0], argPeri_deg[0], a[0],\
                                                        Mass1=1, Mass2=1, verbose=False)
...
#    print '&& The value of paramSettingsDict["simulate_StarPlanet"] = '+repr(paramSettingsDict["simulate_StarPlanet"])
    ...
    for filename in filenames:
        ...
        #[nu,printStr] = findNuFromLog(logFilename)
        s = s#+'\n'+printStr
        print s
        log.write(s+'\n')
        log.close()
...
#    a = 24.82015
    ...
    if True:
        ...
        if True:
            ...
            orbitEllipsePlotterDuo(bestOrbit[0],bestOrbit[1],bestOrbit[3],bestOrbit[4],bestOrbit[5],bestOrbit[6], sysDataDict, DIdataDict,\
                            xLabel='E ["]', yLabel='N ["]', \
        ...
        if False:
            ...
            orbitEllipsePlotterDuo(longAns,es,Ps,incs,argPeris,As, sysDataDict, DIdataDict,\
                            xLabel='E ["]', yLabel='N ["]', \
    ...
    if False:
        ...
        #xlabel = 'Argument of Perigie [deg]'
        paramColNum = 5
        #(CLevels,data,chiSquareds,bestDataVal) = confLevelFinderNEWdataVersion(outputDataFilename,paramColNum, returnData=True, returnChiSquareds=True, returnBestDataVal=True)
        #print 'Argument of Perigie data loaded'
        
        yLabel = 'Inclination [deg]'
        paramColNum = 4
        (CLevels,yData,chiSquareds,bestDataVal) = confLevelFinderNEWdataVersion(outputDatafile,paramColNum, returnData=True, returnChiSquareds=True, returnBestDataVal=True)
        print 'Inclination data loaded' 
                    
        #xlabel = 'Longitude of Ascending Node [deg]'
        paramColNum = 0
        #(CLevels,data,chiSquareds,bestDataVal) = confLevelFinderNEWdataVersion(outputDatafile,paramColNum, returnData=True, returnChiSquareds=True, returnBestDataVal=True)
        ...
        xLabel = 'Period [Years]'
        paramColNum = 3
        (CLevels,xData,chiSquareds,bestDataVal) = confLevelFinderNEWdataVersion(outputDatafile,paramColNum, returnData=True, returnChiSquareds=True, returnBestDataVal=True)
...
def PostSimCompleteAnalysisFunc(outputDatafile=''):
    ...
    if True:      
        ...
        if False:
            ...
            rvModDatasetMakerDuo(e, T, period, inc, argPeri_deg, a, sysDataDict, RVdataDict, paramSettingsDict,\
        ...
        if False:
            ...
            orbitEllipsePlotterDuo(longANs,es,periods,incs,argPeris,a_totals,\
...
def rvPlotterDuo(e, T, Tc, period, inc, argPeri_deg, a, sysDataDict, RVdataDict, paramSettingsDict,\
...
def rvModDatasetMakerDuo(e, T_lastPeri, period, inc, argPeri_deg, a, sysDataDict, RVdataDict, paramSettingsDict,\
...
def singleParamReadAndPlotNEW2(filename, paramColNum, subPlot, numBins, weight=True, confLevels=True, normalize=True, drawLine=False, nu=1):
    ...
    # get data and conf levels
    (data,confLevels) = confLevelFinderNEWdataVersion(filename,paramColNum,True)
